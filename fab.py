...
ENTRY = 'main.py'
VERSION_SRC = []
EXTRA = ['--collect-all', 'pdfminer', '--collect-all', 'pdfplumber', '--collect-all', 'chardet', '--hidden-import', 'PyPDF2']
REPO = 'https://github.com/zzhjim02/CathayExtract'


def version():
    for src in [ENTRY] + VERSION_SRC:
        try:
            t = open(src, encoding='utf-8').read()
        except Exception:
            continue
        for line in t.splitlines():
            if any(k in line for k in ('APP_VERSION', 'APP_TITLE', 'root.title')):
...
- `CathayExtract.exe` —— 单文件版，约 %.0f MB，双击即用
- `runtime\\`（约 185 MB，自带 Python）+ `启动.bat`
...
""" % (v, time.strftime('%Y-%m-%d'), REPO, v, size, '\n'.join(lines))
    ...
    if os.path.exists(p) and os.path.getsize(p) > 200:      # 已经手写过的正文不覆盖
